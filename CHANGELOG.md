...
- Исправлено: сообщения больше не пропадают в редких вкладках. История была одним буфером,
  который резался по общему для всех вкладок лимиту, поэтому поток из шумной вкладки выносил
  всё, что накопили тихие, а пропажа вскрывалась при переключении на них. Теперь у каждой
  вкладки своя глубина истории, а буфер держит ровно столько, сколько во вкладках настроено
- Переключение вкладок не тормозит на большой истории: принадлежность сообщения вкладке
  запоминается при получении и пересчитывается только когда меняются вкладки или их фильтры

## 1.0.5 (русский)

- Исправлено: пунктирная граница закрытого чата больше не закрывает строку сообщений — в
  покое остаются только короткие засечки у краёв панели, а полный пунктир возвращается,
  стоит подвести к нему курсор, так что схватить границу по-прежнему легко
- Рамки и ручки притушены, пока с ними не работают; панель под курсором держит прежнюю
  яркость, так что видно, какую именно собираешься растянуть
